...
if(isset($sujeto->articulos) and count($sujeto->articulos)>=1){
	...
	if (isset($sujeto->pedido)){
		...
		if($pedido->procPedSesion() == 'TRUE'){
			...
			$mail->Subject    = "Orden N#: ".$sujeto->pedido." - Importadora La Villa de las Mascotas, C.A.";
			...
			$mail->AddAddress($_SESSION['user']->name,$sujeto->nombre);
			$mail->AddAddress('ventas@villadelasmascotas.com','Carolina Albornoz');
			...
			if(!$mail->Send()) {
				...
				if(isset($_SESSION['cliente'])){
					...
					unset($_SESSION['cliente']->articulos);	
				}
				if(isset($_SESSION['vendedor'])){
					unset($_SESSION['vendedor']->pedido);
					unset($_SESSION['vendedor']->articulos);
					unset($_SESSION['vendedor']->cliente);
				}					
			}
		}else{
			var_dump($pedido);
